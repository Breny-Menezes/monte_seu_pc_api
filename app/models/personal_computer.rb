class PersonalComputer < ApplicationRecord
  ...
  def has_ram_memories_associated?
    errors.add(:base, "The PC has to have a RAM memory associated with it.") if self.personal_computer_ram_memories.reject { |ram| ram.marked_for_destruction? }.empty?
  end

  def motherboard_is_compatible_with_processing_unit?
    if self.motherboard.supported_processing_unit != 'both'
      errors.add(:base, "The Motherboard is incompatible with the selected processing unit.") if self.processing_unit.brand != self.motherboard.supported_processing_unit
    end
  end

  def ram_memories_quantity_fits_slots_motherboard?
    errors.add(:base, "The PC has only #{motherboard.available_ram_slots} RAM slots available.") if self.personal_computer_ram_memories.count > motherboard.available_ram_slots
  end

  def ram_memories_size_fits_max_ram_motherboard?
    errors.add(:base, "The PC doesn't support more than #{motherboard.max_ram_supported}GB.") if self.personal_computer_ram_memories.map(&:ram_memory).sum(&:size) > motherboard.max_ram_supported
  end
end
